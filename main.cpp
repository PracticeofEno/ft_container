...
void testInserEraseAdvanced(void)
{
	...
	}
	{
		...
		for (int i = 0; i < 10; i++)
		{
			...
			if (std_it == std_c0.end())
				...
				testBool(ft_it->first == std_it->first && ft_it->second == std_it->second, "", 0, 0);
			testMap(ft_c0, std_c0, NOPRINT);
